...
func TestExpiryPolicyCalculates72HoursFromServerTime(t *testing.T) {
	now := time.Date(2026, time.July, 16, 8, 30, 0, 987, time.FixedZone("test", 5*60*60+30*60))
	expiresAt, err := DefaultExpiryPolicy().ExpiresAt("72h", now)
	if err != nil {
		t.Fatal(err)
	}
	want := time.Date(2026, time.July, 19, 3, 0, 0, 0, time.UTC)
	if !expiresAt.Equal(want) || expiresAt.Location() != time.UTC {
		t.Fatalf("ExpiresAt() = %v, want %v", expiresAt, want)
	}
}
...
func TestExpiryPolicyRejectsDurationBeyondMVP(t *testing.T) {
	if _, err := NewExpiryPolicy(map[string]time.Duration{"later": 73 * time.Hour}); err == nil {
